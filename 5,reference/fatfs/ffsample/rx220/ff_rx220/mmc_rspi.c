...
/* RSPI configurations */
#define RSPI_A	2	/* Use RSPIA attached on 1=PA5:PA7, 2=PC5:PC7 */
...
#define FCLK_FAST() {					\
	RSPI.SPCR.BIT.SPE = 0;				\
	RSPI.SPBR = F_PCLK/2/SCLK_FAST-1;	\
	RSPI.SPCR.BIT.SPE = 1;				\
}


/* GPIO configurations */
#define	CS_LOW()	PORTC.PODR.BIT.B4 = 0	/* Set CS Low */
#define	CS_HIGH()	PORTC.PODR.BIT.B4 = 1	/* Set CS High */
#define	INS			(!PORTC.PIDR.BIT.B3)	/* Test for existence of card (True=Inserted, False=Empty, True for default) */
#define	WPRT		0 						/* Test for write protection (True=Write inhibited, False=Write enabled, False for default) */
#define CTRL_INIT()	{	/* Initialize CS,INS,WP */	\
	CS_HIGH();										\
...
void power_on (void)
{
	...
	MPC.PA7PFS.BYTE = 0x0D; PORTA.PMR.BIT.B7 = 1;	/* MISO -> PA7 */
	PORTA.PCR.BIT.B7 = 1;	/* A pull-up on MISO */
#elif RSPI_A == 2
	MPC.PC5PFS.BYTE = 0x0D; PORTC.PMR.BIT.B5 = 1;	/* SCLK -> PB5 */
	MPC.PC6PFS.BYTE = 0x0D; PORTC.PMR.BIT.B6 = 1;	/* MOSI -> PB6 */
	...
	RSPI.SPCMD0.WORD = 0x0700;	/* Set 8-bit, SPI-mode 0, MSB-1st (LSBF=0, SPB=7, BRDV=0, CPOL=0, CPHA=0) */
	RSPI.SPBR = F_PCLK / 2 / SCLK_SLOW - 1;	/* Bit rate */
	RSPI.SPCR.BYTE = 0xC9;		/* Start RSPI in 3-wire master mode (SPRIE=1, SPE=1, MSTR=1, SPMS=1) */
}
...
)
{
	...
	while (!SPRI) ;			/* Wait for end of transfer */

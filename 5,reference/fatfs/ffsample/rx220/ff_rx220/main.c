...
BYTE Buff[8192];			/* Disk/File working buffer */
...
void Excep_CMT0_CMI0 (void)		/* This is an ISR. vect.h is required. */
{
	static uint16_t b;
	...
	/* Blink LED1(P17) on the board */
	b++;
	PORT1.PODR.BIT.B7 = ((b & 0x40) && (b & 0x680) == 0) ? 1 : 0;
...
void ioinit (void)
{
	...
	/* Initialize CMT0 as 1kHz interval timer */
	...
	CMT0.CMCR.WORD = 0x0040;			/* Enable irq and set clock divider (CMIE=1, CKS=0) */
	CMT0.CMCOR = F_PCLK / 8 / 1000 - 1;	/* Set compare value */
	CMT.CMSTR0.BIT.STR0 = 1;			/* Start CMT0 */
	IPR(CMT0, CMI0) = 8;				/* Interrupt priority = 8 */
	IEN(CMT0, CMI0) = 1;				/* Enable CMT0 irq */
...
int main (void)
{
	...
	xputs("\nFatFs module test monitor for RX220\n");
